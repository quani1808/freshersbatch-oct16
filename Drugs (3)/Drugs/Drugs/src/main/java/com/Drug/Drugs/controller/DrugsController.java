...
public class DrugsController {
	...
	public Optional<Drugs> getDrugsById(@PathVariable("drugsId") int drugsId) throws CustomException {
		return service.getDrugsByID(drugsId);
	}
	@GetMapping("/drugs-name/{drugsname}")
	public List<Drugs> getDrugsByName(@PathVariable("drugsname") String drugsName)
	{
		return service.getDrugsByName(drugsName);
	}
	@PutMapping("/update/{id}")
	public Drugs updateDrugsDetails(@PathVariable int id, @Valid @RequestBody Drugs drugs)throws CustomException
	{
		return service.updateDrugsDetails(id, drugs);
	}
	@DeleteMapping("/{id}")
	public String deleteDrugs(@PathVariable("id") int drugsId)throws CustomException
	{
